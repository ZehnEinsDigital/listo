...
OWN connections, NOT part of Listo. Get the sales numbers from **Listo's own Amazon endpoint** (below) — it
pulls them straight from Amazon. Do NOT silently pull sales from another connected service; if you ever need
something Listo can't give, ASK the user first (name the service + what you'd pull) and use it only after a
clear "yes".
...
2. Pull the REAL numbers from Amazon — GET `{LISTO_API_URL}/v1/amazon/sales?credential_id=<id>&marketplace=DE`
   with Bearer `LISTO_API_KEY` (optional `&start=<ISO>&end=<ISO>`; default ≈ last 30 days). The server pulls
