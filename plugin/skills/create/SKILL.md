...
- Fetch the user's marketplaces + connections silently:
  `curl -s -H "Authorization: Bearer $LISTO_API_KEY" "$LISTO_API_URL/v1/me/marketplaces"` (key/url from
  `~/.listo/.env`). It returns each supported marketplace with `connected`/`locked` and, per marketplace,
...
cd ~/.listo/engine && env LISTO_GATEWAY=true \
  ...
  LISTO_CREDENTIAL_ID=<chosen credential_id> LISTO_PLATFORM=<marketplace> LISTO_MARKETPLACE=<country> \
  $(grep -E '^LISTO_API_(KEY|URL)=' ~/.listo/.env | xargs) \
  <command>
```
`LISTO_CREDENTIAL_ID` is mandatory whenever the user has more than one connection — the cloud refuses to
guess otherwise (fail-closed isolation).

- **After the pick, provision quietly (no output to the user):**
  `LISTO_PLATFORM=<marketplace> python3 "${CLAUDE_PLUGIN_ROOT}/scripts/bootstrap.py"` — it pulls the engine
...
- **CP0 — Setup:** Ask for the brand only if not given. Marketplace = the connection picked in step 0b
  ...
  **Work inside the user's CURRENT project folder** — `$CLAUDE_PROJECT_DIR` (fall back to the cwd). Do NOT
  ...
  `~/.listo/engine/runs/`**. Each request is a **fresh** run; do not reuse a previous run's analysis or
  ...
  **Then perform CP0 setup with ONE deterministic command — never by hand** (with the pinned gateway env
  ...
  ```
  cd ~/.listo/engine && env LISTO_CREDENTIAL_ID=<chosen> LISTO_PLATFORM=<marketplace> LISTO_MARKETPLACE=<country> \
    python3 core/cp0_setup.py "<project>/listo-run/<id>" <marketplace> "<feed-path>"
  ```
