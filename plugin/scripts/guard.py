#!/usr/bin/env python3
"""Listo plugin PreToolUse guard (defense-in-depth, NOT the security boundary).

The real safety boundary is server-side: Amazon credentials and the live SP-API write
live only in the Listo cloud, so the client physically cannot perform a live write.
...
NOTE: it does NOT block writes under LISTO_ENGINE_DIR — that points at the customer's
...
def evaluate(tool_name: str, tool_input: dict) -> str | None:
    ...
    if _SP_API in blob:
        return ("Direct Amazon SP-API access is not allowed from the plugin. Listing reads "
                "and writes go through the Listo cloud gateway (your credentials never leave it).")
    if tool_name == "Bash":
        cmd = str((tool_input or {}).get("command", ""))
        if _PY_SKIP.search(cmd):
            return "python with -S/-E/-I would skip the Listo safety shim."
    return None
