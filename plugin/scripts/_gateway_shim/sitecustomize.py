...
When the plugin runs the engine on a customer machine with ``LISTO_GATEWAY=true`` (and
...
the Listo cloud, so NO local ``AMAZON_SP_*`` credentials are needed.

Inert unless ``LISTO_GATEWAY=true``. Must never raise — a failing ``sitecustomize`` would
break the interpreter; activation is wrapped so it can only ever no-op on failure.
"""

import os

if os.environ.get("LISTO_GATEWAY", "").strip().lower() == "true":
    try:
