#!/usr/bin/env python3
"""Listo plugin preflight — run before any engine checkpoint.

Verifies config, pulls the LIVE brain bundle from the Listo cloud (gated by the
...
lk_live_ Listo API key — never an Amazon or Anthropic credential.
...
DEFAULT_ENGINE_DIR = pathlib.Path.home() / ".listo" / "engine"  # auto-provisioned snapshot
...
# from the engine project (all SP-API flows through the Listo cloud gateway).
...
def load_dotenv() -> None:
    ...
    ~/.listo/.env. Shell-exported vars win (setdefault).

    ~/.listo/.env is the GUI path: when the plugin is installed via Upload/Marketplace it
    lives in a managed dir, so the user has no obvious cwd .env — /listo:connect writes the
    key to ~/.listo/.env instead (see save_api_key).
    """
...
def save_api_key(key: str, api_url: str | None = None) -> pathlib.Path:
    """Persist the Listo key to ~/.listo/.env (chmod 600) — used by /listo:connect so a
    GUI user never edits a dotfile by hand. Refuses anything that isn't an lk_live_ key."""
    key = (key or "").strip()
    if not key.startswith("lk_live_"):
        raise SystemExit("LISTO: that is not a Listo key (it must start with lk_live_). "
                         "Get it on the Connect page of the Listo web app.")
    dest = _config_env_path()
    dest.parent.mkdir(parents=True, exist_ok=True)
    lines = [f"LISTO_API_KEY={key}"]
    if api_url and api_url.strip():
        lines.append(f"LISTO_API_URL={api_url.strip()}")
    dest.write_text("\n".join(lines) + "\n", encoding="utf-8")
    try:
        os.chmod(dest, 0o600)
    except OSError:
...
def validate_config(env: dict) -> list[str]:
    ...
    Only LISTO_API_KEY is required. LISTO_ENGINE_DIR is OPTIONAL: unset = the plugin
    ...
    """
    errors: list[str] = []
    key = env.get("LISTO_API_KEY", "")
    if not key.startswith("lk_live_"):
        errors.append("No Listo API key found. Run /listo:connect once to paste your lk_live_ key "
                      "(it is saved to ~/.listo/.env).")
    engine_dir = env.get("LISTO_ENGINE_DIR", "").strip()
    if engine_dir and not pathlib.Path(engine_dir).expanduser().is_dir():
        errors.append("LISTO_ENGINE_DIR is set but is not a directory (unset it to auto-provision).")
    return errors
...
def assemble_injected_rules(bundle: dict) -> str:
    ...
    """
    parts = [f"# Listo injected rules (brain {bundle.get('version_hash', '?')[:12]})", ""]
...
def fetch_brain(api_url: str, api_key: str, marketplace: str = "amazon") -> dict:
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:  # noqa: S310 (trusted Listo host)
            return json.loads(resp.read())
    except urllib.error.HTTPError as exc:
        if exc.code == 402:
            raise SystemExit("LISTO: no active subscription (402). Reactivate your plan to run.")
        raise SystemExit(f"LISTO: brain fetch failed (HTTP {exc.code}).")
    except Exception as exc:  # noqa: BLE001
        raise SystemExit(f"LISTO: could not reach the Listo API: {exc}")
...
def connect_via_browser(api_url: str, *, open_browser: bool = True, sleep=None, max_wait: int = 600):
    ...
    Opens the Listo web app, the user clicks Authorize, and we receive the freshly-minted key
    by polling — then save it to ~/.listo/.env. Returns the config path.
    """
    import time
    import webbrowser
    ...
    print(
        "LISTO: opening your browser to connect. If it doesn't open, go to:\n"
    ...
    while waited < max_wait:
        try:
            ...
            if exc.code == 410:
                raise SystemExit("LISTO: this connection expired before you approved it. Run /listo:connect again.")
            raise SystemExit(f"LISTO: connect failed (HTTP {exc.code}).")
        except Exception as exc:  # noqa: BLE001
            raise SystemExit(f"LISTO: could not reach the Listo API: {exc}")
        ...
        sleep(interval)
        waited += interval
    raise SystemExit("LISTO: authorization timed out. Run /listo:connect again.")
    ...
    with tarfile.open(fileobj=io.BytesIO(blob), mode="r:gz") as tar:
        for m in tar.getmembers():
            ...
            if target != dest and not str(target).startswith(str(dest) + os.sep):
                raise SystemExit(f"LISTO: refusing unsafe path in engine snapshot: {m.name}")
            safe.append(m)
...
def provision_engine(api_url: str, api_key: str, dest: pathlib.Path, marketplace: str = "amazon") -> pathlib.Path:
    ...
    No LISTO_ENGINE_DIR needed: the plugin fetches the engine itself, like the brain.
    """
    dest.mkdir(parents=True, exist_ok=True)
    ...
    try:
        with urllib.request.urlopen(req, timeout=120) as resp:  # noqa: S310 (trusted Listo host)
            blob = resp.read()
        ...
        if exc.code == 402:
            raise SystemExit("LISTO: no active subscription (402). Reactivate your plan to run.")
        raise SystemExit(f"LISTO: engine fetch failed (HTTP {exc.code}).")
    except Exception as exc:  # noqa: BLE001
        raise SystemExit(f"LISTO: could not reach the Listo API: {exc}")
...
def harden_engine_claude_settings(engine_dir: pathlib.Path) -> pathlib.Path:
    ...
    When the user opens ``~/.listo/engine`` as a Claude Code project, its
    ...
    through the Listo cloud gateway). Re-applied on every provision; creates a minimal file
...
def seed_engine(bundle: dict, engine_dir: pathlib.Path) -> list[str]:
    ...
    adapter reads data/amazon_cache/<market>/. LISTO_ENGINE_DIR must be a WRITABLE copy —
    ...
    try:
        ...
        existing.setdefault("version", "listo-served")
        ...
        raise SystemExit(
            f"LISTO: could not seed the brain into {engine_dir}/data — is LISTO_ENGINE_DIR a WRITABLE "
...
def main() -> None:
    ...
    if errors:
        raise SystemExit("LISTO PREFLIGHT FAILED:\n  - " + "\n  - ".join(errors))
    ...
    override = env.get("LISTO_ENGINE_DIR", "").strip()
    if override:
    ...
    print(
        f"LISTO ready: brain {bundle.get('version_hash', '?')[:12]} model={bundle.get('model', '?')} "
